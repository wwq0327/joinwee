...
def index(request):
    ...
    if request.GET.get('order', '') == 'featured':
        lessons = WEELesson.objects.published().fine().select_related('creater')
        ...
        lessons = [h.content_object for h in hits if h.content_object.is_draft==False]
         ...
         lessons = WEELesson.objects.published().filter(level=_level).select_related('creater')
    else:
        lessons = WEELesson.objects.published().select_related('creater')
